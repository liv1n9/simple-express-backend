...
exports.get = async (req, res, next) => {
    try {
        ...
        const data = await billModel
            ...
            .skip(opt.skip)
            .limit(opt.limit)
            .populate("user", "username fullName dateOfBirth email phoneNumber address gender");
...
exports.getById = async (req, res, next) => {
    try {
        ...
        const data = await billModel.findById(id)
            .populate("user", "username fullName dateOfBirth email phoneNumber address gender")
            .populate({ path: "productsList.product", populate: { path: "productType", select: "-_id -updatedAt -createdAt -__v" } });
...
exports.userUpdateById = async (req, res, next) => {
    try {
        ...
        const data = await billModel.findOneAndUpdate(conditions, update, { new: true, runValidators: true });
...
exports.deleteById = async (req, res, next) => {
    try {
        const id = req.params.id;
        const update = req.body;
        const data = await billModel.findByIdAndUpdate(id, update, { new: true, runValidators: true });
        res.status(200).json(data);
    } catch (err) {
        return next(err);
    }
